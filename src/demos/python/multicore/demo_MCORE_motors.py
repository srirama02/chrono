...
import pychrono.mcore as mcore
import pychrono.opengl as opengl
import math as m
...
def CreateSliderGuide(material, sys, pos) :
    ...
    mslider.SetPos(pos + chrono.ChVectorD(0, 0.3, 0))
...
# just to simplify the creation of multiple linear motors in this demo
# (skip this and go to main() for the tutorial)
def CreateStatorRotor(material, sys, pos) :
    ...
    mrotor.SetPos(pos + chrono.ChVectorD(0.5, 0, -0.15))
...
# Create a Chrono multicore physical sys
sys = mcore.ChSystemMulticoreNSC()

# Modify some setting of the physical system
sys.Set_G_acc(chrono.ChVectorD(0, -9.81, 0))
sys.ChangeSolverType(mcore.ChSolverSettingsMulticore.Type_BB)
...
vis = opengl.ChVisualSystemOpenGL()
vis.AttachSystem(sys)
vis.SetWindowTitle('Demo motros')
vis.SetWindowSize(1024,768)
vis.SetRenderMode(opengl.RenderMode_SOLID)
vis.Initialize()
vis.SetCameraPosition(chrono.ChVectorD(1, 3, -7), chrono.ChVectorD(0, 0, 0))
